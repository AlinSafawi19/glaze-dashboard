...
export const TRANSFERS: Record<TransferKey, TransferSpec> = {
  ...
  products: {
    ...
    sample: {
      ...
      Benefits: "Deeply hydrates, softens fine lines, evens tone.",
      "How to Use": "Massage over damp skin morning and night, then rinse.",
      "New in": "no",
      Limited: "yes",
      Brand: "COSRX",
      Categories: "Cleansers, Toners",
      Collection: "Bundles",
    ...
    notes: [
      ...
      "Categories and Skin Types take more than one: pick one from the dropdown, then type the rest after it separated by commas — “Cleansers, Toners”. Excel only offers one value at a time, so that column accepts typing as well as picking.",
...
async function importProducts(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const data = {
      ...
      bestFor: cell(row, "Best For") || null,
      benefits: cell(row, "Benefits") || null,
      howToUse: cell(row, "How to Use") || null,
